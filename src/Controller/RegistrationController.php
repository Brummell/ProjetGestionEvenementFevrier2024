    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, EntityManagerInterface $entityManager,UserRepository $userRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($userRepository->count([]) === 0) {
                // The first user, give all rights
                $user->setRoles(['ROLE_USER', 'ROLE_SUPER_ADMIN']);
                $user->setCanAssignRoles(true);
            } else {
                // Not the first user, give only ROLE_USER
                $user->setRoles(['ROLE_USER']);
                $user->setCanAssignRoles(false);
            }
            ...
            $this->emailVerifier->sendEmailConfirmation('app_verify_email', $user,
                (new TemplatedEmail())
                    ->from(new Address('infos@brummell83.serv00.net', 'Verification'))
                    ->to($user->getEmail())
